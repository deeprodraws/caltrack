...
function IconSearchDots({ size = 15 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
      ...
      title={
        <>
          <h3 style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            {phase === 'capture' && <><IconCamera size={16} /> Scan Food Photo</>}
            {phase === 'scanning' && <><IconSearchDots size={16} /> Analyzing…</>}
            {phase === 'review' && <><IconCheckCircle size={16} /> Review AI Results</>}
            {phase === 'error' && <><IconWarning size={16} /> Scan Failed</>}
          </h3>
