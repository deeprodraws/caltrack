  ...
  try {
    ...
    if (q && q.trim()) {
      result = await pool.query(
        'SELECT * FROM saved_foods WHERE name ILIKE $1 ORDER BY name ASC LIMIT 20',
        [`%${q.trim()}%`]
      );
    } else {
      result = await pool.query('SELECT * FROM saved_foods ORDER BY name ASC');
    }
  ...
  try {
    ...
    const { rows, rowCount } = await pool.query(
      `UPDATE saved_foods SET name=$1, calories=$2, protein=$3, carbs=$4, fat=$5,
       serving_size=$6, serving_unit=$7, tags=$8 WHERE id=$9 RETURNING *`,
      [name, calories, protein, carbs, fat, serving_size, serving_unit, tags ?? '', req.params.id]
    );
...
router.delete('/:id', async (req, res) => {
  try {
    const { rowCount } = await pool.query(
      'DELETE FROM saved_foods WHERE id = $1',
      [req.params.id]
    );
    if (rowCount === 0) return res.status(404).json({ error: 'Not found' });
