...
// Returns the day's metrics + yesterday's food totals
router.get('/', async (req, res) => {
  ...
  try {
    const [mRes, yRes] = await Promise.all([
      pool.query('SELECT * FROM daily_metrics WHERE date = $1', [date]),
      (() => {
        // Compute yesterday's date safely at noon to avoid DST edge cases
        ...
        return pool.query(
          `SELECT
           ...
           FROM food_entries WHERE date = $1`,
          [yesterday]
        );
      })(),
    ]);
...
// PUT /api/metrics
// Upserts only the fields provided in the body
router.put('/', async (req, res) => {
  ...
  try {
    const { rows: [row] } = await pool.query(
      `INSERT INTO daily_metrics (date, steps, water_ml, sleep_hours, updated_at)
       VALUES ($1, COALESCE($2, 0), COALESCE($3, 0), COALESCE($4, 0), NOW())
       ON CONFLICT (date) DO UPDATE SET
         steps       = CASE WHEN $2 IS NOT NULL THEN $2 ELSE daily_metrics.steps       END,
         water_ml    = CASE WHEN $3 IS NOT NULL THEN $3 ELSE daily_metrics.water_ml    END,
         sleep_hours = CASE WHEN $4 IS NOT NULL THEN $4 ELSE daily_metrics.sleep_hours END,
         updated_at  = NOW()
       RETURNING *`,
      [date, s, w, sl]
    );
